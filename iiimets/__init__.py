...
def loadManifestURLsFromPickle(
    url: str, cwd: Path, http: requests.session, fname: str, filterstring: str, logger
) -> list:
    """
...
def start(
    ...
    if update == True:
        ...
        if Path(cwd, "cache", "ids_of_generated_mets.txt").exists():
            ...
            logger.info(f'{str(Path(cwd, "cache", "ids_of_generated_mets.txt"))} erstellt.')
            alreadygeneratedids = []
    else:
        alreadygeneratedids = []
...
def main():
    ...
    if file is None and url is None:
        sys.exit(
            "You need either an URL to an IIIF Collection or a the Path to a file containg links to IIIF Manifests. Try `iiimets --help` for more Information."
        )
    elif file is not None:
        if file.endswith(".txt"):
            newspaper_urls = [line.rstrip("\n") for line in open(file)]
        else:
            newspaper_urls = loadManifestURLsFromPickle(
                url, cwd, http, file, filterstring, logger
            )
    elif url is not None:
        logger.info(f"Getting Newspaper URLs from {url}")
        newspaper_urls = getNewspaperManifests(url, http, filterstring, cwd, logger)

    if len(newspaper_urls) == 0:
        logger.warning("No URLs present")
        sys.exit()

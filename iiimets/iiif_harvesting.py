            ...
            try:
                ...
                if response.status_code == 404:
                    ...
                    try:
                        ...
                        logger.info(f"Identifier Harvesting beendet. Insgesamt {len(manifests)} IDs bekommen.")
                        break
        # manifests is a list of dictionaries with keys `url` and `name` of resource
        return manifests

def getListOfManifests(url: str, session: requests.Session, filter: str, cwd: Path, logger) -> list:
    '''
    ...
    if filter is not None:
        ...
        manifest_urls = df.query(f'name.str.contains("{filter}")', engine="python")['url'].to_list()
    else:
